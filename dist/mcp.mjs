...
import { deleteMasters, fetchReport, getCompanyContext, importMasters, invokeTallyAction, queryCollection, renameObjectArrayProperties } from './tally.mjs';
...
export async function registerMcpServer() {
    ...
    mcpServer.registerTool('company-context', {
        ...
        try {
            return { content: [{ type: 'text', text: JSON.stringify(await getCompanyContext()) }] };
        }
        catch (err) {
    ...
    mcpServer.registerTool('company-profile', { title: 'Company Profile', description: 'Reads the active company structure: ledgers, groups, voucher types, stock items, units and godowns.', inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: false } }, async () => {
        try {
            return { content: [{ type: 'text', text: JSON.stringify(await companyProfile()) }] };
        }
        catch (err) {
            return { isError: true, content: [{ type: 'text', text: String(err) }] };
        }
    ...
    mcpServer.registerTool('voucher-draft-create', { title: 'Create Voucher Draft', description: voucherInputDescription, inputSchema: voucherSchema, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async (args) => { try {
        return { content: [{ type: 'text', text: JSON.stringify(await createDraft(args)) }] };
    }
    catch (err) {
        return { isError: true, content: [{ type: 'text', text: String(err) }] };
    } });
    mcpServer.registerTool('voucher-draft-preview', { title: 'Preview Voucher Draft', description: 'Returns a human-readable draft before posting.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: true, openWorldHint: false } }, async ({ draftId }) => { try {
        return { content: [{ type: 'text', text: JSON.stringify(previewDraft(draftId)) }] };
    }
    catch (err) {
        return { isError: true, content: [{ type: 'text', text: String(err) }] };
    } });
    mcpServer.registerTool('voucher-draft-approve', { title: 'Approve Voucher Draft', description: 'Marks a draft approved. Requires explicit owner or authorized user approval.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async ({ draftId }) => { try {
        return { content: [{ type: 'text', text: JSON.stringify(await approveDraft(draftId)) }] };
    }
    catch (err) {
        return { isError: true, content: [{ type: 'text', text: String(err) }] };
    } });
    mcpServer.registerTool('voucher-draft-post', { title: 'Post Approved Voucher', description: 'Posts an approved voucher to the same verified company and refuses if the active company changed.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: true, idempotentHint: false } }, async ({ draftId }) => { try {
        return { content: [{ type: 'text', text: JSON.stringify(await postDraft(draftId)) }] };
    }
    catch (err) {
        return { isError: true, content: [{ type: 'text', text: String(err) }] };
    } });
    mcpServer.registerTool('voucher-draft-discard', { title: 'Discard Voucher Draft', description: 'Discards an unposted draft.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: true, idempotentHint: true } }, async ({ draftId }) => { try {
        return { content: [{ type: 'text', text: JSON.stringify(await discardDraft(draftId)) }] };
    }
    catch (err) {
        return { isError: true, content: [{ type: 'text', text: String(err) }] };
    } });
