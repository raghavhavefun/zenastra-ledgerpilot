...
export async function getCompanyContext() {
    const companies = await queryCollection('Company', ['Name', 'BooksFrom', 'StateName', 'CountryName', 'IsActiveCompany'], new Map());
    const active = companies.find((company) => company.IsActiveCompany === true);
    if (!active)
        throw new Error('No active Tally company found');
...
export async function createVoucher(voucher, targetCompany) {
    if (!voucher.entries.length)
        throw new Error('At least one ledger entry is required');
    if (voucher.entries.some((entry) => !Number.isFinite(entry.amount) || entry.amount <= 0))
        throw new Error('Voucher entries must contain valid non-zero amounts');
    const date = voucher.date.replace(/-/g, '');
    if (!/^\d{8}$/.test(date))
        throw new Error('Voucher date must be YYYY-MM-DD');
