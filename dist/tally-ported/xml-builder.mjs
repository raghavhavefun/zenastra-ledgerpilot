...
export class XmlBuilder {
    ...
    static buildExportRequest(reportName, staticVars = {}) {
        return `<ENVELOPE>
  <HEADER>
    <VERSION>1</VERSION>
  ...
  <BODY>
    ...
    // ── Collection Export ─────────────────────────────────────────
    static buildCollectionExportRequest(collectionName, staticVars = {}) {
        return `<ENVELOPE>
  <HEADER>
    <VERSION>1</VERSION>
    <TALLYREQUEST>Export</TALLYREQUEST>
  ...
  <BODY>
    ...
    static buildInlineTdlCollectionRequest(collectionName, tdlMessage, staticVars = {}) {
        return `<ENVELOPE>
  <HEADER>
    <VERSION>1</VERSION>
    <TALLYREQUEST>Export</TALLYREQUEST>
  ...
  <BODY>
    ...
    // ── Object Export ─────────────────────────────────────────────
    static buildObjectExportRequest(subtype, id, fetchFields = [], staticVars = {}) {
  ...
  <BODY>
    ...
    // ── Function Evaluation ───────────────────────────────────────
    static buildFunctionRequest(functionId, tdlMessage, params = []) {
  ...
  <BODY>
    ...
    // ── Import Data (Masters) ─────────────────────────────────────
    static buildMasterImportRequest(masterXml, company) {
  ...
  <BODY>
    ...
    static buildVoucherImportRequest(voucherXml, company) {
        const companyVar = company
            ? `<SVCURRENTCOMPANY>${this.esc(company)}</SVCURRENTCOMPANY>`
            : '';
        return `<ENVELOPE>
  ...
  <BODY>
    ...
    static buildActionRequest(actionId) {
        return `<ENVELOPE>
  <HEADER>
    <VERSION>1</VERSION>
  ...
  <BODY><DESC></DESC></BODY>
    ...
    static buildLedgerXml(params) {
        ...
        if (params.opening_balance !== undefined && params.opening_balance !== null)
        ...
        if (params.is_bill_by_bill !== undefined)
            xml += `\n    <ISBILLWISEON>${params.is_bill_by_bill ? 'Yes' : 'No'}</ISBILLWISEON>`;
        if (params.credit_limit)
    ...
    // ── Delete Master XML ─────────────────────────────────────────
    static buildDeleteMasterXml(type, name) {
    ...
    static buildGroupXml(params) {
        ...
        xml += `\n    <NAME>${this.esc(params.new_name || params.name)}</NAME>`;
        xml += `\n    <PARENT>${this.esc(params.parent)}</PARENT>`;
        if (params.is_subledger !== undefined)
            xml += `\n    <ISSUBLEDGER>${params.is_subledger ? 'Yes' : 'No'}</ISSUBLEDGER>`;
        if (params.affects_gross_profit !== undefined)
    ...
    static buildStockItemXml(params) {
        ...
        if (params.is_batch_wise !== undefined)
            xml += `\n    <ISBATCHWISEON>${params.is_batch_wise ? 'Yes' : 'No'}</ISBATCHWISEON>`;
        if (params.is_expiry_applicable !== undefined)
    ...
    static buildGodownXml(params) {
        ...
        if (params.address)
            xml += `\n    <ADDRESS.LIST><ADDRESS>${this.esc(params.address)}</ADDRESS></ADDRESS.LIST>`;
        if (params.allow_storage !== undefined)
    ...
    static buildCostCategoryXml(params) {
        ...
        xml += `\n    <NAME>${this.esc(params.name)}</NAME>`;
        if (params.allocate_revenue !== undefined)
            xml += `\n    <ALLOCATEREVENUE>${params.allocate_revenue ? 'Yes' : 'No'}</ALLOCATEREVENUE>`;
        if (params.allocate_non_revenue !== undefined)
    ...
    static buildStockGroupXml(params) {
        ...
        if (params.under)
            xml += `\n    <PARENT>${this.esc(params.under)}</PARENT>`;
        if (params.should_quantities_add !== undefined)
    ...
    // ── Price Level XML ───────────────────────────────────────────
    static buildPriceLevelXml(name) {
    ...
    static buildEmployeeXml(params) {
        ...
        if (params.date_of_joining)
            xml += `\n    <DATEOFJOINING>${params.date_of_joining}</DATEOFJOINING>`;
        if (params.date_of_leaving)
    ...
    static buildVoucherXml(params) {
        ...
        if (params.is_inventory_on)
            xml += `\n    <ISINVENTORYON>Yes</ISINVENTORYON>`;
        if (params.place_of_supply)
        ...
        if (params.payroll_entries) {
            for (const pe of params.payroll_entries) {
                ...
                if (pe.is_deemed_positive !== undefined)
    ...
    static buildPayHeadXml(params) {
        ...
        if (params.base_pay_head)
            xml += `\n    <COMPUTEDON>${this.esc(params.base_pay_head)}</COMPUTEDON>`;
        if (params.computation_formula)
        ...
        if (params.affect_net_salary !== undefined)
    ...
    static buildCompanyXml(params) {
        ...
        xml += `\n    <NAME>${this.esc(params.name)}</NAME>`;
        if (params.financial_year_from)
            xml += `\n    <STARTINGFROM>${params.financial_year_from}</STARTINGFROM>`;
        if (params.books_beginning_from)
...
//# sourceMappingURL=xml-builder.mjs.map