...
export function registerFinancialReportTools(server, client, state) {
    let count = 0;
    const sv = (from, to) => ({
    ...
    server.tool('tally_get_trial_balance', 'Fetch the trial balance for a date range. Returns all ledger/group balances with debit/credit totals.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_profit_and_loss', 'Fetch the Profit & Loss statement for a date range.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_balance_sheet', 'Fetch the Balance Sheet as of a given date.', { as_of_date: z.string().describe('Balance sheet date (YYYYMMDD)') }, async (params) => {
        try {
    ...
    server.tool('tally_get_cash_flow', 'Fetch the cash flow statement for a date range.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_funds_flow', 'Fetch the funds flow (working capital changes) statement for a date range.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_ledger_vouchers', 'Fetch all vouchers posted to a specific ledger within a date range (ledger statement).', {
    ...
    server.tool('tally_get_group_summary', 'Fetch summary of all ledgers under a specific group with balances.', {
    ...
    count++;
    // ── 8. tally_get_daybook ──────────────────────────────────────
    server.tool('tally_get_daybook', 'Fetch the daybook (all vouchers for a single day or date range).', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_cash_book', 'Fetch the cash book (all cash transactions) for a date range.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_bank_book', 'Fetch the bank book (all bank account transactions) for a date range.', {
        ...
        try {
            ...
            if (params.bank_ledger)
                vars['DSPLEDGER'] = params.bank_ledger;
    ...
    server.tool('tally_get_ratio_analysis', 'Fetch key financial ratios (current ratio, debt-equity, ROI etc.) from Tally.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_budget_variance', 'Get budget vs actual variance for a specified budget and date range.', {
    ...
    server.tool('tally_get_cost_centre_report', 'Get a cost-centre-wise break-up of income/expenses.', {
        cost_centre: z.string().optional().describe('Specific cost centre (omit for all)'),
        ...dateRangeSchema,
    }, async (params) => {
        try {
            const vars = sv(params.from_date, params.to_date);
            if (params.cost_centre)
                vars['DSPCOSTCENTRE'] = params.cost_centre;
    ...
    server.tool('tally_get_profit_centre_report', 'Get profit-centre-wise profitability statement.', {
        profit_centre: z.string().optional().describe('Specific profit centre (omit for all)'),
        ...dateRangeSchema,
    }, async (params) => {
        try {
            const vars = sv(params.from_date, params.to_date);
            if (params.profit_centre)
                vars['DSPCOSTCENTRE'] = params.profit_centre;
    ...
    server.tool('tally_get_statistics', 'Get company-level statistics: voucher counts by type, master counts, accounts info.', dateRangeSchema, async (params) => {
        try {
...
//# sourceMappingURL=reports-financial.mjs.map