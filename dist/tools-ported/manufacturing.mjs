...
export function registerManufacturingTools(server, client, state) {
    let count = 0;
    ...
    // ── 1. tally_create_bom ──────────────────────────────────────
    server.tool('tally_create_bom', 'Create a Bill of Materials (BOM) for a stock item — list of components with quantities.', {
    ...
    count++;
    // ── 2. tally_alter_bom ───────────────────────────────────────
    server.tool('tally_alter_bom', 'Update an existing BOM for a stock item.', {
    ...
    count++;
    // ── 3. tally_get_bom ─────────────────────────────────────────
    server.tool('tally_get_bom', 'Get the BOM components for a stock item.', { stock_item_name: z.string().describe('Stock item name') }, async (params) => {
        try {
    ...
    server.tool('tally_create_manufacturing_journal', 'Create a manufacturing journal — consume raw materials and produce finished goods.', {
    ...
    server.tool('tally_get_production_report', 'Get production report showing items manufactured in a period.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_consumption_report', 'Get raw material consumption report for a period.', {
    ...
    server.tool('tally_get_job_costing', 'Get job costing report — cost centre wise allocation of manufacturing costs.', {
    ...
    server.tool('tally_get_work_order_status', 'Get work order / production order status — pending vs completed.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_bom_requirements', 'Calculate raw material requirements based on BOM for a given production quantity.', {
    ...
    server.tool('tally_stock_transfer_between_godowns', 'Transfer stock between godowns via a stock journal.', {
        date: z.string().describe('Date (YYYYMMDD)'),
    ...
    server.tool('tally_get_wastage_report', 'Get wastage / scrap report from manufacturing processes.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_yield_analysis', 'Get yield analysis — actual vs expected output from manufacturing.', {
    ...
    count++;
    // ── 13. tally_list_boms ──────────────────────────────────────
    server.tool('tally_list_boms', 'List all stock items that have BOMs defined.', {}, async () => {
        try {
    ...
    server.tool('tally_get_process_costing', 'Get process costing report — cost accumulation through manufacturing stages.', {
...
//# sourceMappingURL=manufacturing.mjs.map