...
export function registerGstTools(server, client, state) {
    let count = 0;
    const sv = (from, to) => ({
    ...
    // ── 1. tally_get_gstr1 ───────────────────────────────────────
    server.tool('tally_get_gstr1', 'Get complete GSTR-1 return data (all outward supplies) for a period.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_gstr1_b2b', 'Get GSTR-1 B2B section: invoices to registered dealers.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_gstr1_b2c', 'Get GSTR-1 B2C section: sales to unregistered consumers.', dateRangeSchema, async (params) => {
        try {
            const config = TDLBuilder.buildCustomCollection({
                collectionType: 'Voucher',
    ...
    server.tool('tally_get_gstr1_cdnr', 'Get GSTR-1 CDNR section: credit/debit notes to registered parties.', dateRangeSchema, async (params) => {
        try {
            const config = TDLBuilder.buildCustomCollection({
                collectionType: 'Voucher',
                fetchFields: ['Date', 'VoucherNumber', 'PartyledgerName', 'Amount', 'PartyGSTIN', 'VoucherTypeName'],
    ...
    count++;
    // ── 5. tally_get_gstr2a ──────────────────────────────────────
    server.tool('tally_get_gstr2a', 'Get GSTR-2A data (auto-populated inward supplies from Tally).', dateRangeSchema, async (params) => {
        try {
    ...
    count++;
    // ── 6. tally_get_gstr2b ──────────────────────────────────────
    server.tool('tally_get_gstr2b', 'Get GSTR-2B data (ITC available as per auto-drafted statement).', dateRangeSchema, async (params) => {
        try {
    ...
    count++;
    // ── 7. tally_get_gstr3b ──────────────────────────────────────
    server.tool('tally_get_gstr3b', 'Get GSTR-3B summary return data for a tax period.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_hsn_summary', 'Get HSN/SAC code wise summary of outward and inward supplies.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_gst_tax_liability', 'Get GST tax liability: CGST, SGST, IGST, Cess breakdown.', dateRangeSchema, async (params) => {
        try {
            const config = TDLBuilder.buildCustomCollection({
    ...
    server.tool('tally_get_gst_itc', 'Get Input Tax Credit (ITC) details — eligible, ineligible, reversed.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_gst_reconciliation', 'Get GST reconciliation: books vs returns, mismatch identification.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_generate_einvoice', 'Generate e-Invoice JSON for a specific sales voucher (for IRN generation).', {
    ...
    server.tool('tally_generate_eway_bill', 'Generate e-Way Bill JSON data for a voucher.', {
        voucher_number: z.string().describe('Voucher number'),
        voucher_date: z.string().describe('Voucher date (YYYYMMDD)'),
    ...
    server.tool('tally_get_gst_ims', 'Get GST Invoice Management System (IMS) data.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_gst_notices', 'Get list of GST notices or exceptions flagged by Tally.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_verify_gstin', 'Validate a GSTIN format and look up the party in Tally master data.', {
        gstin: z.string().describe('15-character GSTIN to verify'),
    }, async (params) => {
        try {
            const validation = validateGSTIN(params.gstin);
            if (!validation.valid)
                return result({ status: 'validation_error', message: validation.error });
            const config = TDLBuilder.buildCustomCollection({
                collectionType: 'Ledger',
    ...
    server.tool('tally_get_gst_return_status', 'Get status of GST return preparation and filing readiness.', {
    ...
    server.tool('tally_get_gst_exceptions', 'Get GST-related exceptions: mismatches in GSTIN, HSN, tax rates, place of supply etc.', dateRangeSchema, async (params) => {
        try {
