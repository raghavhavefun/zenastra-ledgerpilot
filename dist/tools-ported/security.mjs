...
export function registerSecurityTools(server, client, state) {
    let count = 0;
    ...
    // ── 1. tally_create_user ──────────────────────────────────────
    server.tool('tally_create_user', 'Create a Tally user with specific security level / access rights.', {
    ...
    count++;
    // ── 2. tally_alter_user ───────────────────────────────────────
    server.tool('tally_alter_user', 'Update an existing Tally user (change security level, password, etc.).', {
    ...
    count++;
    // ── 3. tally_delete_user ──────────────────────────────────────
    server.tool('tally_delete_user', 'Delete a Tally user.', { name: z.string().describe('Username to delete') }, async (params) => {
        try {
    ...
    count++;
    // ── 4. tally_list_users ───────────────────────────────────────
    server.tool('tally_list_users', 'List all Tally users and their security levels.', {}, async () => {
        try {
    ...
    server.tool('tally_get_audit_log', 'Get audit trail / activity log — who changed what and when.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_altered_vouchers_log', 'Get list of vouchers that were altered/modified (for audit purposes).', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_deleted_vouchers_log', 'Get list of deleted/cancelled vouchers (for audit trail).', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_exception_reports', 'Get data exception reports — negative stock, zero-value vouchers, unusual entries.', {
