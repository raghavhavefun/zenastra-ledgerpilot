...
export function registerOutstandingReportTools(server, client, state) {
    let count = 0;
    const sv = (from, to) => ({
    ...
    server.tool('tally_get_receivables', 'Get total receivables summary from Sundry Debtors with party breakup.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_payables', 'Get total payables summary from Sundry Creditors with party breakup.', dateRangeSchema, async (params) => {
        try {
            const config = TDLBuilder.buildCustomCollection({
                collectionType: 'Ledger',
                fetchFields: ['Name', 'ClosingBalance', 'OpeningBalance', 'Parent'],
    ...
    server.tool('tally_get_bills_outstanding', 'Get bill-wise outstanding for a specific party ledger.', {
    ...
    server.tool('tally_get_overdue_receivables', 'Get overdue receivables — bills past due date from Sundry Debtors.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_overdue_payables', 'Get overdue payables — bills past due date from Sundry Creditors.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_party_outstanding', 'Get consolidated outstanding for a specific party (debtor or creditor).', {
        party_ledger: z.string().describe('Party ledger name'),
        ...dateRangeSchema,
    }, async (params) => {
        try {
    ...
    server.tool('tally_get_ageing_analysis', 'Get ageing analysis (0-30, 31-60, 61-90, 90+ days) for receivables or payables.', {
        ...
        try {
            const type = params.group === 'Sundry Debtors' ? 'Debtors' : 'Creditors';
    ...
    server.tool('tally_get_interest_calculation', 'Get simple interest calculation on outstanding balances.', {
        party_ledger: z.string().describe('Party ledger name'),
...
//# sourceMappingURL=reports-outstanding.mjs.map