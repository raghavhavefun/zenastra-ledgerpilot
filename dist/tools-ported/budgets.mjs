...
export function registerBudgetTools(server, client, state) {
    let count = 0;
    ...
    server.tool('tally_create_budget', 'Create a budget with allocations for ledgers/groups by period.', {
    ...
    server.tool('tally_alter_budget', 'Update an existing budget allocations.', {
        name: z.string().describe('Budget name'),
        allocations: z.array(z.object({
            name: z.string().describe('Ledger or group name'),
    ...
    server.tool('tally_delete_budget', 'Delete a budget.', { name: z.string().describe('Budget name to delete') }, async (params) => {
        try {
    ...
    server.tool('tally_list_budgets', 'List all budgets defined in the company.', {}, async () => {
        try {
    ...
    server.tool('tally_get_budget_detail', 'Get details of a specific budget including all allocations.', { budget_name: z.string().describe('Budget name') }, async (params) => {
        try {
    ...
    server.tool('tally_get_budget_vs_actual', 'Get budget vs actual comparison report.', {
    ...
    server.tool('tally_get_budget_utilization', 'Get budget utilization percentage — how much of the budget has been consumed.', {
        budget_name: z.string().describe('Budget name'),
        ...dateRangeSchema,
    }, async (params) => {
        try {
    ...
    server.tool('tally_get_scenario_report', 'Get scenario-based report (actual, budget, what-if) for financial planning.', {
        ...
        try {
            const vars = sv(params.from_date, params.to_date);
            if (params.scenario)
                vars['DSPSCENARIO'] = params.scenario;
...
//# sourceMappingURL=budgets.mjs.map