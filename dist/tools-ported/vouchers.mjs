...
export function registerVoucherTools(server, client, state) {
    let count = 0;
    const getCompany = () => state.activeCompany || undefined;
    // ── 1. tally_create_sales_invoice ─────────────────────────────
    server.tool('tally_create_sales_invoice', 'Create a sales invoice. Handles party ledger, sales ledger, GST, inventory items, bill refs, cost centres. Follow Tally amount sign conventions: Party Dr (negative amount, ISDEEMEDPOSITIVE=Yes), Sales Cr (positive amount, ISDEEMEDPOSITIVE=No).', {
    ...
    server.tool('tally_create_purchase_invoice', 'Create a purchase invoice. Party Cr (negative amount, ISDEEMEDPOSITIVE=No), Purchase Dr (positive amount, ISDEEMEDPOSITIVE=Yes).', {
        date: z.string().describe('Invoice date (YYYYMMDD)'),
    ...
    server.tool('tally_create_receipt', 'Create a receipt voucher (money received). Cash/Bank Dr, Party Cr.', {
    ...
    server.tool('tally_create_payment', 'Create a payment voucher (money paid out). Party Dr, Cash/Bank Cr.', {
    ...
    server.tool('tally_create_journal', 'Create a journal voucher with multiple debit/credit entries. Entries must balance.', {
    ...
    server.tool('tally_create_contra', 'Create a contra voucher — transfer between cash and bank accounts.', {
    ...
    server.tool('tally_create_credit_note', 'Create a credit note (sales return). Reverses a sales invoice.', {
    ...
    server.tool('tally_create_debit_note', 'Create a debit note (purchase return). Reverses a purchase invoice.', {
    ...
    server.tool('tally_create_stock_journal', 'Create a stock journal for inter-godown transfer or stock adjustment.', {
    ...
    server.tool('tally_create_physical_stock', 'Create a physical stock voucher for stock verification/adjustment.', {
        date: z.string().describe('Date (YYYYMMDD)'),
        items: z.array(z.object({
            stock_item_name: z.string().describe('Stock item'),
    ...
    server.tool('tally_create_reversing_journal', 'Create a reversing journal that auto-reverses on a specified date.', {
        date: z.string().describe('Journal date (YYYYMMDD)'),
        entries: z.array(z.object({
    ...
    server.tool('tally_create_memorandum_voucher', 'Create a memorandum/optional voucher that does not affect books.', {
        date: z.string().describe('Date (YYYYMMDD)'),
    ...
    server.tool('tally_alter_voucher', 'Modify an existing voucher — update narration and other details.', {
    ...
    server.tool('tally_cancel_voucher', 'Cancel a voucher (marks it cancelled but keeps it in the system).', {
    ...
    server.tool('tally_delete_voucher', 'Permanently delete a voucher from TallyPrime.', {
        voucher_number: z.string().describe('Voucher number'),
        voucher_type: z.string().describe('Voucher type'),
        date: z.string().describe('Voucher date (YYYYMMDD)'),
    }, async (params) => {
    ...
    server.tool('tally_list_vouchers', 'List vouchers filtered by date range, type, and/or party ledger.', {
