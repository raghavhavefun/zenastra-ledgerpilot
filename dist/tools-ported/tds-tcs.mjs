...
export function registerTdsTcsTools(server, client, state) {
    let count = 0;
    const sv = (from, to) => ({
    ...
    server.tool('tally_get_tds_computation', 'Get TDS computation report — deductions made, due amounts, rates applied.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_tds_outstanding', 'Get TDS outstanding report — deducted but not yet deposited.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_tds_payable_report', 'Get TDS payable summary — section-wise and party-wise breakdown.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_tcs_computation', 'Get TCS computation report — collection amounts, rates applied per party.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_tcs_outstanding', 'Get TCS outstanding — collected but not deposited.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_form_26q', 'Get Form 26Q data — quarterly TDS return for non-salary payments.', {
    ...
    server.tool('tally_get_form_27q', 'Get Form 27Q data — quarterly TDS return for payments to NRIs.', {
        quarter: z.enum(['Q1', 'Q2', 'Q3', 'Q4']).describe('Quarter'),
        financial_year: z.string().describe('Financial year (e.g. "2023-24")'),
        ...dateRangeSchema,
    }, async (params) => {
    ...
    count++;
    // ── 8. tally_get_form_16 ─────────────────────────────────────
    server.tool('tally_get_form_16', 'Get Form 16/16A data for certificate generation.', {
    ...
    server.tool('tally_verify_pan_tds', 'Validate PAN format and check if the party exists in Tally with valid PAN.', {
    ...
    server.tool('tally_get_tds_challan', 'Get TDS challan details for a period (for deposit tracking).', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_lower_deduction_certificates', 'List lower/nil deduction certificates recorded in Tally.', dateRangeSchema, async (params) => {
        try {
    ...
    server.tool('tally_get_tds_nature_summary', 'Get summary of TDS by nature of payment (section-wise: 194A, 194C, etc.).', dateRangeSchema, async (params) => {
        try {
...
//# sourceMappingURL=tds-tcs.mjs.map