...
export type VoucherDraft = {
  ...
  createdAt: string; postedResult?: unknown; idempotencyKey?: string; approvedAt?: string; postedAt?: string;
};
const drafts = new Map<string, VoucherDraft>();
const auditLog: Array<{id:string; action:string; draftId?:string; company?:string; at:string}> = [];
const stateDir = process.env.LEDGERPILOT_STATE_DIR || path.join(process.env.HOME || '.', '.zenastra-ledgerpilot');
const stateFile = path.join(stateDir, 'state.json');
let stateLoaded = false;
async function loadState() { if (stateLoaded) return; stateLoaded = true; try { const state = JSON.parse(await readFile(stateFile, 'utf8')); for (const d of state.drafts || []) drafts.set(d.id, d); auditLog.push(...(state.audit || [])); } catch {} }
...
function audit(action:string, draft?:VoucherDraft) { auditLog.push({ id:crypto.randomUUID(), action, draftId:draft?.id, company:draft?.company, at:new Date().toISOString() }); }
...
export async function createDraft(input: Omit<VoucherDraft, 'id'|'status'|'createdAt'|'postedResult'>) {
  await loadState();
  if (input.idempotencyKey) { const existing = [...drafts.values()].find(d => d.idempotencyKey === input.idempotencyKey); if (existing) return { draft: existing, validation: { valid: true, reused: true }, companyProfile: { company: existing.company } }; }
  ...
  drafts.set(id, draft); audit('draft-created', draft); await saveState();
  return { draft, validation: { valid: true, ...totals }, companyProfile: profile.context };
}

export async function getDraft(id: string) { await loadState(); const draft = drafts.get(id); if (!draft) throw new Error(`Draft '${id}' not found`); return draft; }
export async function approveDraft(id: string) { const draft = await getDraft(id); if (draft.status !== 'draft') throw new Error(`Draft is already ${draft.status}`); draft.status = 'approved'; draft.approvedAt = new Date().toISOString(); audit('draft-approved', draft); await saveState(); return draft; }
export async function postDraft(id: string) {
  const draft = await getDraft(id); if (draft.status !== 'approved') throw new Error('Draft must be approved before posting');
  ...
  const result = await createVoucher(draft, draft.company); draft.status = 'posted'; draft.postedAt = new Date().toISOString(); draft.postedResult = result; audit('draft-posted', draft); await saveState(); return { draft, result };
}
export async function discardDraft(id: string) { const draft = await getDraft(id); if (draft.status === 'posted') throw new Error('Posted drafts cannot be discarded; cancel/reverse in Tally'); drafts.delete(id); audit('draft-discarded', draft); await saveState(); return { id, discarded: true }; }
export async function previewDraft(id: string) { const draft = await getDraft(id); return { ...draft, instructions: 'Review company, voucher type, ledgers, amounts, date and narration before approval.' }; }
