...
export async function getCompanyContext(): Promise<any> {
    const companies = await queryCollection('Company', ['Name', 'BooksFrom', 'StateName', 'CountryName', 'IsActiveCompany'], new Map<string, string>());
    const active = companies.find((company) => company.IsActiveCompany === true);
    if (!active) throw new Error('No active Tally company found');
    return { company: active.Name, booksFrom: active.BooksFrom, state: active.StateName, country: active.CountryName, availableCompanies: companies.map((company) => company.Name) };
}

export async function createVoucher(voucher: { voucherType: string; date: string; voucherNumber?: string; partyLedger?: string; narration?: string; entries: { ledger: string; amount: number; isDeemedPositive: boolean }[] }, targetCompany?: string): Promise<m.CreateUpdateDeleteStatus> {
    if (!voucher.entries.length) throw new Error('At least one ledger entry is required');
    if (voucher.entries.some((entry) => !Number.isFinite(entry.amount) || entry.amount <= 0)) throw new Error('Voucher entries must contain valid non-zero amounts');
    const date = voucher.date.replace(/-/g, '');
    if (!/^\d{8}$/.test(date)) throw new Error('Voucher date must be YYYY-MM-DD');
    const escapeXml = (value: string) => value.replace(/[<>&"']/g, (char) => ({ '<': '&lt;', '>': '&gt;', '&': '&amp;', '"': '&quot;', "'": '&apos;' }[char] as string));
    const ledgerEntries = voucher.entries.map((entry) => `<ALLLEDGERENTRIES.LIST><LEDGERNAME>${escapeXml(entry.ledger)}</LEDGERNAME><ISDEEMEDPOSITIVE>${entry.isDeemedPositive ? 'Yes' : 'No'}</ISDEEMEDPOSITIVE><AMOUNT>${entry.isDeemedPositive ? -Math.abs(entry.amount) : Math.abs(entry.amount)}</AMOUNT></ALLLEDGERENTRIES.LIST>`).join('');
    const xml = `<ENVELOPE><HEADER><TALLYREQUEST>Import Data</TALLYREQUEST></HEADER><BODY><IMPORTDATA><REQUESTDESC><REPORTNAME>Vouchers</REPORTNAME>${targetCompany ? `<STATICVARIABLES><SVCURRENTCOMPANY>${escapeXml(targetCompany)}</SVCURRENTCOMPANY></STATICVARIABLES>` : ''}</REQUESTDESC><REQUESTDATA><TALLYMESSAGE><VOUCHER VCHTYPE="${escapeXml(voucher.voucherType)}" ACTION="Create"><DATE>${date}</DATE>${voucher.voucherNumber ? `<VOUCHERNUMBER>${escapeXml(voucher.voucherNumber)}</VOUCHERNUMBER>` : ''}<VOUCHERTYPENAME>${escapeXml(voucher.voucherType)}</VOUCHERTYPENAME>${voucher.partyLedger ? `<PARTYLEDGERNAME>${escapeXml(voucher.partyLedger)}</PARTYLEDGERNAME>` : ''}${voucher.narration ? `<NARRATION>${escapeXml(voucher.narration)}</NARRATION>` : ''}${ledgerEntries}</VOUCHER></TALLYMESSAGE></REQUESTDATA></IMPORTDATA></BODY></ENVELOPE>`;
    const response = await postTallyXML(xml);
    const resultObj = new XMLParser().parse(response);
    return resultObj['RESPONSE'] || resultObj;
}
