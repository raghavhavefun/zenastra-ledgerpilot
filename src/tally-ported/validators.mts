...
import { TallyState } from './state.mjs';
...
export function validateDate(date: string): { valid: boolean; error?: string } {
  ...
  if (month < 1 || month > 12) return { valid: false, error: `Invalid month: ${month}` };
  if (day < 1 || day > 31) return { valid: false, error: `Invalid day: ${day}` };
  if (year < 1900 || year > 2100) return { valid: false, error: `Invalid year: ${year}` };
...
export function validateLedgersExist(names: string[], state: TallyState): string[] {
  return names.filter(name => !state.ledgerExists(name));
}

export function validateStockItemsExist(names: string[], state: TallyState): string[] {
...
export function sanitizeAmount(amount: any): number {
  ...
  if (typeof amount === 'string') {
    ...
    if (isNaN(parsed)) throw new Error(`Cannot parse amount: ${amount}`);
    return parsed;
  }
