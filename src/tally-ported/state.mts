...
import { TallyClient } from './client.mjs';
import { TDLBuilder } from './tdl-builder.mjs';
import { XmlParser } from './xml-parser.mjs';
import logger from './logger.mjs';

export class TallyState {
  ledgerNames: Set<string> = new Set();
  stockItemNames: Set<string> = new Set();
  voucherTypes: Set<string> = new Set();
  groupNames: Set<string> = new Set();
  godownNames: Set<string> = new Set();
  costCentreNames: Set<string> = new Set();
  unitNames: Set<string> = new Set();
  employeeNames: Set<string> = new Set();
  batchNames: Map<string, Set<string>> = new Map();
  ...
  constructor(client: TallyClient, cacheTtlMs: number = 300000, company?: string) {
    this.client = client;
    this.cacheTtlMs = cacheTtlMs;
    this.company = company;
  }

  getStaticVars(): Record<string, string> {
  ...
  async refreshStockItems(): Promise<void> {
    try {
  ...
  async refreshGroups(): Promise<void> {
    try {
      const config: import('./tdl-builder.mjs').CollectionConfig = {
        name: 'MCPCacheGroups',
        type: 'Group',
        nativeMethods: ['Name'],
      };
  ...
  async refreshVoucherTypes(): Promise<void> {
    try {
      const config: import('./tdl-builder.mjs').CollectionConfig = {
  ...
  async refreshGodowns(): Promise<void> {
    try {
      const config: import('./tdl-builder.mjs').CollectionConfig = {
  ...
  async refreshCostCentres(): Promise<void> {
    try {
      const config: import('./tdl-builder.mjs').CollectionConfig = {
  ...
  async refreshActiveCompany(): Promise<void> {
    try {
  ...
  async refreshAfterMasterChange(entityType: string): Promise<void> {
    switch (entityType.toLowerCase()) {
      case 'ledger': await this.refreshLedgers(); break;
      case 'stockitem': case 'stock_item': await this.refreshStockItems(); break;
      case 'group': await this.refreshGroups(); break;
      case 'vouchertype': case 'voucher_type': await this.refreshVoucherTypes(); break;
      case 'godown': await this.refreshGodowns(); break;
      case 'costcentre': case 'cost_centre': await this.refreshCostCentres(); break;
      case 'unit': await this.refreshUnits(); break;
      case 'employee': await this.refreshEmployees(); break;
      default: break;
    }
  }

  ledgerExists(name: string): boolean {
    return this.ledgerNames.has(name);
  }

  stockItemExists(name: string): boolean {
    return this.stockItemNames.has(name);
  }

  godownExists(name: string): boolean {
    return this.godownNames.has(name);
  }

  employeeExists(name: string): boolean {
