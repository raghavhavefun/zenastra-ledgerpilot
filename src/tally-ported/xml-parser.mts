...
export class XmlParser {
   ...
   */
  static parse(xmlString: string): TallyParseResult {
   ...
   */
  static detectErrors(parsed: any, rawXml: string): string[] {
    const errors: string[] = [];
    ...
    if (rawXml.trimStart().startsWith('<EXCEPTION>')) {
      ...
      if (match) {
        ...
        if (msg) errors.push(`EXCEPTION: ${msg}`);
      } else {
    ...
    if (lineErrorMatch) {
      for (const match of lineErrorMatch) {
        ...
        if (msg) errors.push(`LINEERROR: ${msg}`);
      }
    }
   ...
   */
  static extractImportResult(parsed: any): {
    created: number;
    altered: number;
    deleted: number;
    errors: number;
   ...
   */
  static extractCollectionData(parsed: any): any[] {
   ...
   */
  static extractObjectData(parsed: any): any {
    const envelope = parsed?.ENVELOPE || parsed;
   ...
   */
  static extractNameList(parsed: any, entityTag: string): string[] {
...
export class TallyValueParser {
   ...
   */
  static cleanString(val: string): string {
    if (!val) return '';
    return val
      .replace(/&amp;/g, '&')
      .replace(/&lt;/g, '<')
   ...
   */
  static parseAmount(val: string | number | undefined | null): number {
   ...
   */
  static parseDate(val: string | undefined | null): string | null {
    if (!val) return null;
   ...
   */
  static parseQuantity(val: string | number | undefined | null): { qty: number; unit: string } {
    if (val === undefined || val === null || val === '') return { qty: 0, unit: '' };
   ...
   */
  static parseBool(val: string | boolean | undefined | null): boolean {
   ...
   */
  static parseRate(val: string | number | undefined | null): { rate: number; unit: string } {
    if (val === undefined || val === null || val === '') return { rate: 0, unit: '' };
    if (typeof val === 'number') return { rate: val, unit: '' };
    const s = String(val).trim();
    const parts = s.split('/');
    if (parts.length === 2) {
      return {
