...
import { approveDraft, companyProfile, createDraft, discardDraft, getDraft, listDrafts, postDraft, previewDraft, voucherInputDescription } from './write.mjs';
...
export async function registerMcpServer(): Promise<McpServer> {
  ...
  mcpServer.registerTool('voucher-drafts-list', { title: 'List Voucher Drafts', description: 'Lists drafts held by this MCP process.', inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: false } }, async () => ({ content: [{ type: 'text', text: JSON.stringify(listDrafts()) }] }));
