...
import { createVoucher, deleteMasters, fetchReport, getCompanyContext, importMasters, invokeTallyAction, queryCollection, renameObjectArrayProperties } from './tally.mjs';
import { cacheTable, executeSQL } from './database.mjs';
import { lstCollectionFields, lstOptionCountryState } from './definition.mjs';
import { utility } from './utility.mjs';
import { approveDraft, auditEntries, companyProfile, createDraft, discardDraft, getDraft, listDrafts, postDraft, previewDraft, voucherInputDescription } from './write.mjs';
...
export async function registerMcpServer(): Promise<McpServer> {
  ...
  mcpServer.registerTool('company-context', {
    title: 'Current Company Context',
    description: 'Reads the active Tally company and accounting context. Call before writing.',
    inputSchema: {},
    annotations: { readOnlyHint: true, openWorldHint: false }
  }, async () => {
    try { return { content: [{ type: 'text', text: JSON.stringify(await getCompanyContext()) }] }; }
    catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; }
  });

  mcpServer.registerTool('company-profile', { title: 'Company Profile', description: 'Reads the active company structure: ledgers, groups, voucher types, stock items, units and godowns.', inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: false } }, async () => {
    try { return { content: [{ type: 'text', text: JSON.stringify(await companyProfile()) }] }; } catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; }
  });

  const voucherSchema = {
    company: z.string().optional(), voucherType: z.string().min(1), date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/), voucherNumber: z.string().optional(), partyLedger: z.string().optional(), narration: z.string().optional(),
    entries: z.array(z.object({ ledger: z.string().min(1), amount: z.number().positive(), isDeemedPositive: z.boolean() })).min(2)
  };
  mcpServer.registerTool('voucher-draft-create', { title: 'Create Voucher Draft', description: voucherInputDescription, inputSchema: voucherSchema, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async (args) => { try { return { content: [{ type: 'text', text: JSON.stringify(await createDraft(args)) }] }; } catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; } });
  mcpServer.registerTool('voucher-draft-preview', { title: 'Preview Voucher Draft', description: 'Returns a human-readable draft before posting.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: true, openWorldHint: false } }, async ({ draftId }) => { try { return { content: [{ type: 'text', text: JSON.stringify(previewDraft(draftId)) }] }; } catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; } });
  mcpServer.registerTool('voucher-draft-approve', { title: 'Approve Voucher Draft', description: 'Marks a draft approved. Requires explicit owner or authorized user approval.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: false, idempotentHint: true } }, async ({ draftId }) => { try { return { content: [{ type: 'text', text: JSON.stringify(await approveDraft(draftId)) }] }; } catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; } });
  mcpServer.registerTool('voucher-draft-post', { title: 'Post Approved Voucher', description: 'Posts an approved voucher to the same verified company and refuses if the active company changed.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: true, idempotentHint: false } }, async ({ draftId }) => { try { return { content: [{ type: 'text', text: JSON.stringify(await postDraft(draftId)) }] }; } catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; } });
  mcpServer.registerTool('voucher-draft-discard', { title: 'Discard Voucher Draft', description: 'Discards an unposted draft.', inputSchema: { draftId: z.string() }, annotations: { readOnlyHint: false, openWorldHint: false, destructiveHint: true, idempotentHint: true } }, async ({ draftId }) => { try { return { content: [{ type: 'text', text: JSON.stringify(await discardDraft(draftId)) }] }; } catch (err) { return { isError: true, content: [{ type: 'text', text: String(err) }] }; } });
  mcpServer.registerTool('voucher-drafts-list', { title: 'List Voucher Drafts', description: 'Lists persisted voucher drafts.', inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: false } }, async () => ({ content: [{ type: 'text', text: JSON.stringify(await listDrafts()) }] }));
  mcpServer.registerTool('audit-log', { title: 'Audit Log', description: 'Returns persisted draft approval and posting audit events.', inputSchema: {}, annotations: { readOnlyHint: true, openWorldHint: false } }, async () => ({ content: [{ type: 'text', text: JSON.stringify(await auditEntries()) }] }));
