...
export function registerBulkTools(server: McpServer, client: TallyClient, state: TallyState): number {
  let count = 0;
  const getCompany = () => state.activeCompany || undefined;

  // ── 1. tally_bulk_create_ledgers ──────────────────────────────
  server.tool(
    'tally_bulk_create_ledgers',
    'Create multiple ledgers in one shot. Efficient for initial setup.',
    {
  ...
  server.tool(
    'tally_bulk_create_stock_items',
    'Create multiple stock items in one batch.',
    {
  ...
  server.tool(
    'tally_bulk_create_groups',
    'Create multiple account groups in one batch.',
    {
  ...
  server.tool(
    'tally_bulk_create_vouchers',
    'Import multiple vouchers in one batch (same or different types).',
    {
  ...
  server.tool(
    'tally_bulk_create_cost_centres',
    'Create multiple cost centres in one batch.',
    {
  ...
  server.tool(
    'tally_bulk_create_employees',
    'Create multiple employees in one batch.',
    {
  ...
  server.tool(
    'tally_bulk_create_godowns',
    'Create multiple godowns/warehouses in one batch.',
    {
  ...
  server.tool(
    'tally_bulk_alter_ledgers',
    'Update multiple ledgers in a single batch (change group, balances, etc.).',
    {
      ledgers: z.array(z.object({
  ...
  server.tool(
    'tally_bulk_delete_masters',
    'Delete multiple masters of the same type in one batch.',
    {
  ...
  server.tool(
    'tally_bulk_create_units',
    'Create multiple units of measure in one batch.',
    {
  ...
  server.tool(
    'tally_export_masters_csv_format',
    'Export master data in a flat, JSON-table format (like CSV) for analysis.',
    {
  ...
  server.tool(
    'tally_export_vouchers_flat',
    'Export vouchers in a flat JSON format for analysis or migration.',
    {
    ...
    async (params) => {
      try {
        ...
        if (params.voucher_type) filters.TypeFilter = `$VoucherTypeName = "${params.voucher_type}"`;
        if (params.party_ledger) filters.PartyFilter = `$PartyledgerName = "${params.party_ledger}"`;
