...
export function registerCompanyTools(server: McpServer, client: TallyClient, state: TallyState): number {
  let count = 0;
  ...
  server.tool(
    ...
    'Get comprehensive info about a company — name, address, FY, GST, features enabled.',
    {
    ...
    async (params) => {
      try {
        ...
        if (!companyName) return result({ status: 'error', message: 'No company specified or active' });
  ...
  server.tool(
    'tally_alter_company',
    'Update company configuration — address, statutory info, features.',
    {
  ...
  server.tool(
    ...
    'Test if TallyPrime is running and reachable. Returns version and active company info.',
    {},
    async () => {
      try {
  ...
  server.tool(
    'tally_refresh_cache',
    'Force-refresh the master data cache (ledgers, stock items, groups, etc.).',
    {},
    async () => {
      try {
        await state.refresh();
  ...
  server.tool(
    'tally_get_license_info',
    'Get TallyPrime license information: edition, serial number, features.',
    {},
    async () => {
      try {
  ...
  server.tool(
    'tally_get_feature_status',
    'Check which features are enabled in the active company (F11/F12 settings).',
    {},
    async () => {
      try {
        const companyName = state.activeCompany;
        if (!companyName) return result({ status: 'error', message: 'No active company' });
