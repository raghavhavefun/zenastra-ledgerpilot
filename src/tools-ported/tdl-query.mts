...
export function registerTdlQueryTools(server: McpServer, client: TallyClient, state: TallyState): number {
  let count = 0;
  ...
  server.tool(
    'tally_query_collection',
    'Run a custom TDL collection query against TallyPrime. This is the most flexible query tool — specify object type, fields to fetch, and filter conditions.',
    {
  ...
  server.tool(
    'tally_execute_tdl_inline',
    'Execute raw inline TDL code in TallyPrime. POWER TOOL — for advanced queries not covered by other tools. The TDL snippet is injected inside the XML request.',
    {
    ...
    async (params) => {
      try {
        const response = await client.queryRawTdlCollection(
          params.collection_name || 'MyCollection',
          params.tdl_snippet,
  ...
  server.tool(
    'tally_evaluate_function',
    'Evaluate a TDL function in TallyPrime and get the result. Use for computed values, aggregations, conditionals.',
    {
  ...
  server.tool(
    'tally_export_object',
    'Export a complete Tally object by type and name — returns full XML representation with all fields.',
    {
  ...
  server.tool(
    'tally_export_report',
    'Export any named Tally report as XML. Use when a specific report module tool is not available.',
    {
  ...
  server.tool(
    'tally_execute_action',
    'Execute a TDL action in TallyPrime (e.g. alter company settings, run macros).',
    {
  ...
  server.tool(
    'tally_search_masters',
    'Fuzzy search across all master types: ledgers, stock items, groups, cost centres. Returns matching names.',
    {
  ...
  server.tool(
    'tally_get_master_names',
    'Get a quick list of all names for a given master type. Uses cached data when available.',
    {
  ...
  server.tool(
    'tally_import_raw_xml',
    'Import raw XML into TallyPrime. POWER TOOL — for advanced use when building custom XML payloads.',
    {
  ...
  server.tool(
    'tally_send_raw_xml',
    'Send any raw XML request to TallyPrime and get the raw response. Ultimate flexibility.',
    {
  ...
  server.tool(
    'tally_get_collection_types',
    'Get a reference of available Tally collection types and common fetch fields for building queries.',
    {},
    async () => {
      const reference = {
        Ledger: {
  ...
  server.tool(
    'tally_get_tdl_syntax_help',
    'Get TDL syntax reference for building custom queries and functions. Includes collection syntax, filter patterns, function patterns.',
    {
