...
export function registerOrderTools(server: McpServer, client: TallyClient, state: TallyState): number {
  let count = 0;
  const getCompany = () => state.activeCompany || undefined;

  // ── 1. tally_create_sales_order ───────────────────────────────
  server.tool(
    'tally_create_sales_order',
    'Create a sales order with line items. Tracked until fulfilled via delivery/invoice.',
    {
  ...
  server.tool(
    'tally_create_delivery_note',
    'Create a delivery note (goods dispatched against a sales order).',
    {
  ...
  server.tool(
    'tally_create_receipt_note',
    'Create a receipt note (goods received against a purchase order / GRN).',
    {
      date: z.string().describe('Date (YYYYMMDD)'),
      party_ledger: z.string().describe('Supplier ledger'),
  ...
  server.tool(
    'tally_create_rejection_out',
    'Create a rejection out voucher (return goods to supplier).',
    {
      date: z.string().describe('Date (YYYYMMDD)'),
      party_ledger: z.string().describe('Supplier ledger'),
  ...
  server.tool(
    'tally_create_rejection_in',
    'Create a rejection in voucher (customer returns goods).',
    {
      date: z.string().describe('Date (YYYYMMDD)'),
      party_ledger: z.string().describe('Customer ledger'),
  ...
  server.tool(
    'tally_list_sales_orders',
    'List sales orders within a date range, optionally filtered by party and status.',
    {
  ...
  server.tool(
    'tally_get_order_outstanding',
    'Get outstanding (unfulfilled) orders — sales or purchase.',
    {
  ...
  server.tool(
    'tally_get_sales_order_book',
    'Get the sales order book showing all orders and their fulfillment status.',
    {
      from_date: z.string().describe('Start date (YYYYMMDD)'),
      to_date: z.string().describe('End date (YYYYMMDD)'),
    },
