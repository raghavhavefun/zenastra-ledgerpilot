...
export function registerBankingTools(server: McpServer, client: TallyClient, state: TallyState): number {
  let count = 0;
  ...
  server.tool(
    ...
    'Get bank reconciliation statement for a bank account.',
    {
  ...
  server.tool(
    'tally_reconcile_bank_transaction',
    'Mark a bank transaction as reconciled with bank date.',
    {
  ...
  server.tool(
    'tally_get_unreconciled_transactions',
    'Get all unreconciled bank transactions for a bank account.',
    {
      bank_ledger: z.string().describe('Bank account ledger name'),
      ...dateRangeSchema,
    },
  ...
  server.tool(
    'tally_get_cheque_register',
    'Get cheque register — all cheques issued and received.',
    {
  ...
  server.tool(
    'tally_get_post_dated_cheques',
    'Get list of post-dated cheques (PDC) pending clearing.',
    dateRangeSchema,
    async (params) => {
      try {
  ...
  server.tool(
    'tally_get_deposit_slips',
    'Get deposit slip summary for bank deposits.',
    {
  ...
  server.tool(
    'tally_get_bank_balance',
    'Get current balance of a bank account (book balance).',
    {
      bank_ledger: z.string().describe('Bank account ledger name'),
  ...
  server.tool(
    'tally_list_bank_accounts',
    'List all bank account ledgers with balances.',
    {
  ...
  server.tool(
    'tally_get_payment_performance',
    'Get payment performance analysis — on-time vs delayed payments.',
    dateRangeSchema,
    async (params) => {
      try {
  ...
  server.tool(
    'tally_get_cash_position',
    'Get current cash position — cash in hand + all bank balances.',
    {
