...
export function registerInventoryReportTools(server: McpServer, client: TallyClient, state: TallyState): number {
  let count = 0;
  ...
  server.tool(
    'tally_get_stock_summary',
    'Get overall stock summary: all items with closing quantity and value.',
    dateRangeSchema,
    async (params) => {
      try {
  ...
  server.tool(
    'tally_get_stock_item_detail',
    'Get full details of a specific stock item including batches, godowns, opening/closing balances.',
    {
  ...
  server.tool(
    'tally_get_godown_summary',
    'Get godown-wise stock summary showing quantity and value per godown.',
    dateRangeSchema,
    async (params) => {
      try {
  ...
  server.tool(
    'tally_get_stock_movement',
    'Get stock movement (inward/outward) for a stock item over a period.',
    {
      stock_item_name: z.string().describe('Stock item name'),
      ...dateRangeSchema,
    },
  ...
  server.tool(
    'tally_get_batch_report',
    'Get batch-wise stock report showing quantity per batch for an item.',
    {
  ...
  server.tool(
    'tally_get_expiry_report',
    'Get stock items nearing or past expiry date.',
    {
  ...
  server.tool(
    'tally_get_reorder_report',
    'Get stock items at or below their reorder level.',
    dateRangeSchema,
    async (params) => {
      try {
        const config = TDLBuilder.buildCustomCollection({
  ...
  server.tool(
    'tally_get_stock_valuation',
    'Get stock valuation report — closing value by different valuation methods.',
    dateRangeSchema,
    async (params) => {
      try {
        const config = TDLBuilder.buildCustomCollection({
  ...
  server.tool(
    'tally_get_movement_analysis',
    'Get movement analysis for inventory — inward/outward quantities & values.',
    dateRangeSchema,
    async (params) => {
      try {
  ...
  server.tool(
    'tally_get_category_summary',
    'Get stock category-wise summary with totals.',
    dateRangeSchema,
    async (params) => {
      try {
        const config = TDLBuilder.buildCustomCollection({
